...
import control_insurance.cSpecialInsurance;
import model_insurance.mSpecialInsurance;

public class specialInformation extends JFrame {
   ...
   private cSpecialInsurance cSpecialInsurance;
   private mSpecialInsurance specialInsurance;
   private boolean painted = true;

   private JScrollPane jScrollPane;
   private JScrollPane jScrollPane2;
   private JScrollPane jScrollPane3;
   private JTextArea specialAgree1;
   private JTextArea specialAgree2;
   private JTextArea specialAgree3;
   ...
   public specialInformation() {
      ...
      this.specialInsuranceRegistrationView = new specialInsuranceRegistrationView();
      this.cSpecialInsurance = new cSpecialInsurance();
      this.specialInsurance = new mSpecialInsurance();
   ...
   public void run() {
      this.specialInsurance = this.cSpecialInsurance.insertInfo();

      if (this.specialInsurance.getArraySize()) {
         this.info1 = this.specialInsurance.getSubscriptionTarget();
         this.info2 = this.specialInsurance.getInsurancePeriod();
         this.info3 = Integer.toString(this.specialInsurance.getInsuranceMoney());
         this.info4 = Integer.toString(this.specialInsurance.getInsuranceMoney());
         this.info5 = Integer.toString(this.specialInsurance.getInsuranceMoney());
         this.info6 = this.specialInsurance.getSubscriptionTarget();
         state = true;
      } else {
         JOptionPane.showMessageDialog(null, "가입 가능한 특종보험이 없습니다.");
         this.state = false;
      }

      if (painted == true) {

         this.setTitle("특종보험 정보안내");
         ...
         JLabel titleLabel = new JLabel("<특종보험 정보안내>");
         ...
         String context = "1. 특종보험 주요 내용 계약서";
         ...
         this.specialAgree1 = new JTextArea(context2, 5, 20);
         this.specialAgree1.setEditable(false);
         this.jScrollPane = new JScrollPane(specialAgree1, jScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
         ...
         String sInfor4 = "● 보험" + "\n\r" + "- 이 보험은 특종보험으로써 상해나 여행, 동산과 같은 특별한 경우에 손실이 발생했을 경우 보상해주는 보험입니다."
               + "\n\r" + "- 이 보험의 가입대상은 --입니다. 가입대상에 충족하지 못하는 경우 가입이 거절될 수 있습니다." + "\n\r" + "- 이 보험의 기간은 --입니다"
               + "\n\r" + "- 보험료는 월납이며 계약 기간에 따라 그 보험금이 조금씩 달라집니다." + "\n\r" + "1) 1년~3년(단기보험)일 경우 : --입니다."
               + "\n\r" + "2) 3년~10년(장기보험)일 경우 : --입니다." + "\n\r" + "- 사고로 인한 손실이 발생했을 경우 보험금은 최대 --입니다." + "\n\r"
               + "- 특종보험의 종류에는 [상해보험, 배상책임보험, 도난보험, 레저종합보험, 유아교육기관종합보험, 동산보험, 여행보험, 컨티전시보험]이 있습니다." + "\n\r"
               + "● 보험금 지급 안내" + "\n\r" + "- 인터넷 접수 : 보험홈페이지에서 '2. 고객'을 선택 후 '3. 사고접수'에 들어와서 사고접수를 진행할 수 있습니다."
               + "\n\r" + "사고내용을 최대한 상세하게 적어주셔야 적절한 보험금 지급이 가능합니다." + "\n\r"
               + "접수한 사고내용을 바탕으로 사후처리사가 보험금을 지급할 것입니다." + "\n\r" + "사고 내용을 상세하기 적지 않았을 경우 보험금 지급이 거절될 수 있습니다.";
         ...
         this.specialAgree2 = new JTextArea(sInfor4, 5, 10);
         this.specialAgree2.setEditable(false);
         this.jScrollPane2 = new JScrollPane(specialAgree2, jScrollPane2.VERTICAL_SCROLLBAR_ALWAYS,
         ...
         this.specialAgree3 = new JTextArea(info6, 5, 10);
         this.specialAgree3.setEditable(false);
         this.jScrollPane3 = new JScrollPane(specialAgree3, jScrollPane2.VERTICAL_SCROLLBAR_ALWAYS,
               jScrollPane2.HORIZONTAL_SCROLLBAR_AS_NEEDED);
   ...
   public void getRegistrationInfo() {
      ...
      this.inputTextSize = inputTextSize.length;
   }
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         if (e.getActionCommand() == "다음") {
            ...
            if (answer1 != "동의1" || answer2 != "동의2") {
               ...
               specialInsuranceRegistrationView.getInformation(name, date, inputTextSize);
