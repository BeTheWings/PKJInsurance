...
import control_insuranceDesigning.cSpecialInsurnaceDesigning;
...
public class specialInsuranceDesigningView extends JFrame {
	...
	public void run() {
		if (painted == true) {
			...
			inputText[0].setToolTipText("ex)보험이름");
			...
			inputText[1].setToolTipText("ex)남녀노소");
			...
			inputText[2].setToolTipText("ex)7000");
			...
			inputText[5].setToolTipText("ex)3000000");
			...
			inputText[6].setToolTipText("ex)최근 1년 내 특종보험 관련 사고가 없었던 사람");
	...
	public boolean isBlank() {
		...
		if (insuranceName.equals("")) {
			...
			checker = cSpecialInsurnaceDesigning.checkOverlap(insuranceName); // 보험이름중복
			if (checker) {
				JOptionPane.showMessageDialog(null, "이미 존재하는 보험입니다!");
			} else {
				return true;
			}
		} else {
	...
	private void putText() {
		...
		this.cSpecialInsurnaceDesigning.setData(this.data);
	}
