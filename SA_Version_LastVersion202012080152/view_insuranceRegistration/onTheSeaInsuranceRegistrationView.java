...
import control_insuranceRegistration.cOnTheSeaInsuranceRegistration;
...
public class onTheSeaInsuranceRegistrationView extends JFrame {
	...
	private String boatType, gender, job, longterm, onTheSeaType, accidentHistory, selectAccidentHistory, name, date;
	...
	public onTheSeaInsuranceRegistrationView() {
		...
		this.cOnTheSeaInsuranceRegistration = new cOnTheSeaInsuranceRegistration();
	...
	public void run() {
		if (painted == true) {
			...
			inputLabel = new JLabel("                                                  1.보험 이름을 입력해주세요");
			...
			inputText[0].setToolTipText("ex)해상보험");
			...
			inputLabel = new JLabel("                                                  12. 소유재산을 선택해주세요");
			...
			inputText[8].setToolTipText("ex)동해바다에서 어업을 하다 바다에 빠지는 사고 발생. 병원비 150000원");
	...
	public void putText() {
		...
		if (inputText[8].isEditable() != false) {
			this.list.add(this.inputText[8].getText());// 사고이력
		} else {
			this.list.add(nothing);
		}
		list.add(group1.getSelection().getActionCommand());// 성별
		list.add(group2.getSelection().getActionCommand());// 직업
		list.add(group3.getSelection().getActionCommand());// 장기보험여부
		list.add(group4.getSelection().getActionCommand());// 보트 종류
		list.add(group5.getSelection().getActionCommand());// 보험 종류
		list.add(group6.getSelection().getActionCommand());/// 참고사항

		findPayIn();
		list.add(Integer.toString(this.totalPrice2));
		list.add(this.date);

		this.cOnTheSeaInsuranceRegistration.setData(this.list);
	}

	public void findPayIn() {
		this.totalPrice2 = cOnTheSeaInsuranceRegistration.findPayIn(insuranceName);
	...
	public boolean isBlank() {
		...
		} else if (selectAccidentHistory == null && inputText[6].isEditable() != true) {
			JOptionPane.showMessageDialog(null, "참고사항을 선택해주세요!");
		} else if (accidentHistory.equals("") && inputText[6].isEditable() != false) {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand() == "완료") {
				...
				if (stopper == true) {
					...
					} else if (cOnTheSeaInsuranceRegistration.checkCategory(inputText[0].getText())) { // true여야
						putText();
						dispose();
					} else {
			...
			} else if (e.getActionCommand().equals("선체보험") || e.getActionCommand().equals("선박항해보험")
					|| e.getActionCommand().equals("초과책임보험") || e.getActionCommand().equals("건조보험")
					|| e.getActionCommand().equals("선박불가동손실보험") || e.getActionCommand().equals("운임보험")
					|| e.getActionCommand().equals("전쟁보험")) {
				onTheSeaType = e.getActionCommand();
			} else if (e.getActionCommand().equals("소형선") || e.getActionCommand().equals("중형선")
					|| e.getActionCommand().equals("대형선")) {
				boatType = e.getActionCommand();
