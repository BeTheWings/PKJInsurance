...
import control_insurance.cOnTheSeaInsurance;
import model_insurance.mOnTheSeaInsurance;

public class onTheSeaInformation extends JFrame {
   ...
   private cOnTheSeaInsurance cOnTheSeaInsurance;
   ...
   private JTextArea onTheSeaAgree1;
   private JTextArea onTheSeaAgree2;
   private JTextArea onTheSeaAgree3;
   ...
   public onTheSeaInformation() {
      this.actionHandler = new actionHandler();
      this.onTheSeaInsuranceRegistrationView = new onTheSeaInsuranceRegistrationView();
      this.cOnTheSeaInsurance = new cOnTheSeaInsurance();
   ...
   public void run() {
      this.onTheSeaInsurance = this.cOnTheSeaInsurance.insertInfo();

      if (this.onTheSeaInsurance.getArraySize()) {
         this.info1 = this.onTheSeaInsurance.getSubscriptionTarget();
         this.info2 = this.onTheSeaInsurance.getInsurancePeriod();
         this.info3 = Integer.toString(this.onTheSeaInsurance.getInsuranceMoney());
         this.info4 = Integer.toString(this.onTheSeaInsurance.getInsuranceMoney());
         this.info5 = Integer.toString(this.onTheSeaInsurance.getInsuranceMoney());
         this.info6 = this.onTheSeaInsurance.getSubscriptionTarget();
         this.state = true;
      } else {
         JOptionPane.showMessageDialog(null, "가입 가능한 해상보험이 없습니다.");
         this.state = false;
      }

      if (painted == true) {

         this.setTitle("해상보험 정보안내");
         ...
         JLabel titleLabel = new JLabel("<해상보험 정보안내>");
         ...
         String context = "1. 해상보험 주요 내용 계약서";
         ...
         this.onTheSeaAgree1 = new JTextArea(context2, 5, 20);
         this.onTheSeaAgree1.setEditable(false);
         this.jScrollPane = new JScrollPane(onTheSeaAgree1, jScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
         ...
         String sInfor4 = "● 보험" + "\n\r" + "- 이 보험은 해상보험으로써 주변 환경에 의해 본인의 배에 대한 손실이 발생했을 경우 보상해주는 보험입니다." + "\n\r"
               + "- 이 보험의 가입대상은 " + info1 + "입니다. 가입대상에 충족하지 못하는 경우 가입이 거절될 수 있습니다." + "\n\r" + "- 이 보험의 기간은 "
               + info2 + "입니다" + "\n\r" + "- 보험료는 월납이며 계약 기간에 따라 그 보험금이 조금씩 달라집니다." + "\n\r"
               + "1) 1년~3년(단기보험)일 경우 : " + info3 + "원 입니다." + "\n\r" + "2) 3년~10년(장기보험)일 경우 : " + info4 + "원 입니다."
               + "\n\r" + "- 해상에서의 사고로 인한 손실이 발생했을 경우 보험금은 최대 " + info5 + "원 입니다." + "\n\r"
               + "- 해상보험의 종류에는 [선체보험, 선박항해보험, 초과책임보험, 건조보험, 선박불가동손실보험, 운임보험, 전쟁보험]이 있습니다." + "\n\r" + "● 보험금 지급 안내"
         ...
         this.onTheSeaAgree2 = new JTextArea(sInfor4, 5, 10);
         this.onTheSeaAgree2.setEditable(false);
         this.jScrollPane2 = new JScrollPane(onTheSeaAgree2, jScrollPane2.VERTICAL_SCROLLBAR_ALWAYS,
         ...
         this.onTheSeaAgree3 = new JTextArea(info6, 5, 1);
         this.onTheSeaAgree3.setEditable(false);
         this.jScrollPane3 = new JScrollPane(onTheSeaAgree3, jScrollPane2.VERTICAL_SCROLLBAR_ALWAYS,
               jScrollPane2.HORIZONTAL_SCROLLBAR_AS_NEEDED);
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         if (e.getActionCommand() == "다음") {
            if (answer1 != "동의1" || answer2 != "동의2") {
               ...
               onTheSeaInsuranceRegistrationView.getInformation(name, date, inputTextSize);
