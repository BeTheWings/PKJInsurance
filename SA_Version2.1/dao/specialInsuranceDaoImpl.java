...
public class specialInsuranceDaoImpl extends dao implements insuranceDao {
	...
	public ArrayList<String> insertInfo() {
		...
		List<specialInsuranceVO> output = session.selectList("specialInsuranceMapper.insertInfo", "특종보험");
		this.info = new ArrayList<String>();

		if (output == null) {
			System.out.println("조회결과 없음");
		} else {
			for (specialInsuranceVO item : output) {
	...
	public void insert(insuranceDesingingVO insuranceDesingingVO) {
		...
		insuranceDesingingVO input = insuranceDesingingVO;
