...
public class fireInsuranceRegistrationView extends JFrame {
	...
	private String buildingType, gender, property, job, longterm, selectAccidentHistory, accidentHistory, date, name;
	private int totalPrice;
	private ArrayList<String> list;

	public fireInsuranceRegistrationView() {
		this.cFireInsuranceRegistration = new cFireInsuranceRegistration();
	...
	public void getInforamtion(String name, String date, int totalPrice) {
		this.name = name;
		this.date = date;
	...
	public void run() {
		if (painted == true) {
			...
			inputText[8].setToolTipText("ex)email@naver.com");
			...
			inputText[7].setToolTipText("ex)서울특별시 서대문구 남가좌동 명지대학교 인문생활관에서 화재가 발생함 인명피해는 없으나 재산피해 3000000원");
			panel10_1.add(inputText[7]);
			inputText[7].setEditable(false);
	...
	public void putText() {
		//Model
		//input test ->view에서 얻어온 정보 Model 과 view로 연결
		// Controller호출
		
		ArrayList<String> Data = new ArrayList<String>();
		
		Data.add(inputText[0].getText());
		Data.add(inputText[1].getText());
		Data.add(inputText[2].getText());
		Data.add(inputText[3].getText());
		Data.add(inputText[4].getText());
		Data.add(inputText[5].getText());
		Data.add(inputText[8].getText());
		...
		cFireInsuranceRegistration.setAge(inputText[2].getText());
		...
		cFireInsuranceRegistration.setEmail(inputText[8].getText());
		if (inputText[6].isEditable() != false) {
			cFireInsuranceRegistration.setBuildingType(inputText[6].getText());
		} else {
			cFireInsuranceRegistration.setBuildingType(this.property);
		}
		if (inputText[7].isEditable() != false) {
			cFireInsuranceRegistration.setAccidentHistory(inputText[7].getText());
		} else {
			cFireInsuranceRegistration.setAccidentHistory(this.selectAccidentHistory);
		}
		...
		cFireInsuranceRegistration.setSelectAccidentHistory(group5.getSelection().getActionCommand());

		cFireInsuranceRegistration.setTotalPrice(this.totalPrice);
		cFireInsuranceRegistration.setDate(this.date);
	...
	public boolean isBlank() {
		...
		String eMail = this.inputText[8].getText();

		if (inputText[6].isEditable() != false) {
			buildingType = inputText[6].getText();
		} else {
			buildingType = "나무목재";
		}

		if (inputText[7].isEditable() != false) {
			accidentHistory = inputText[7].getText();
		} else {
		...
		} else if (property == null && inputText[6].isEditable() != true) {
			JOptionPane.showMessageDialog(null, "소유재산을 선택해주세요!");
		} else if (buildingType.equals("") && inputText[6].isEditable() != false) {
			JOptionPane.showMessageDialog(null, "재산을 입력해주세요!");
		} else if (longterm == null) {
			JOptionPane.showMessageDialog(null, "장기보험여부를 선택해주세요!");
		} else if (selectAccidentHistory == null && inputText[7].isEditable() != true) {
			JOptionPane.showMessageDialog(null, "참고사항을 선택해주세요!");
		} else if (accidentHistory.equals("") && inputText[7].isEditable() != false) {
	...
	class actionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand() == "완료") {
				...
				if (inputText[6].isEditable() == true) {
					inputText[6].setEditable(false);
				}
				property = "나무목재";
				...
				inputText[7].setEditable(true);
			} else if (e.getActionCommand().equals("사고 이력 없음")) {
				if (inputText[7].isEditable() == true) {
					inputText[7].setEditable(false);
				}
