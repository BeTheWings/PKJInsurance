...
import control_insuranceRegistration.cFireInsuranceRegistration;
...
public class fireInsuranceRegistrationView extends JFrame {
	...
	public void run() {
		if (painted == true) {
			...
			inputText[9].setToolTipText("ex)서울특별시 서대문구 남가좌동 명지대학교 인문생활관에서 화재가 발생함 인명피해는 없으나 재산피해 3000000원");
	...
	public void putText() {
		...
		this.list.add(this.inputText[6].getText());// 계좌번호
		this.list.add(this.inputText[7].getText());// 이메일
		if (inputText[8].isEditable() != false) {
			this.list.add(this.inputText[8].getText());// 소유재산 입력
		} else {
		...
		list.add(group3.getSelection().getActionCommand());// 소유재산 버튼
		...
		this.cFireInsuranceRegistration.setData(this.list);
	}

	public void setInforamtion(String name, String date, String inputTextSize) {
		this.name = name;
		this.date = date;
	...
	public boolean isBlank() {
		...
		if (inputText[8].isEditable() != false) {
			buildingType = inputText[8].getText();
		} else {
			buildingType = "나무목재";
		}
		...
		} else if (property == null && inputText[8].isEditable() != true) {
			JOptionPane.showMessageDialog(null, "소유재산을 선택해주세요!");
		} else if (buildingType.equals("") && inputText[8].isEditable() != false) {
	...
	public void findPayIn() {
		...
		this.totalPrice2 = cFireInsuranceRegistration.findPayIn(insuranceName);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand() == "완료") {
				...
				if (stopper == true) {
					...
					} else if (cFireInsuranceRegistration.checkCategory(inputText[0].getText())) { // true여야
						System.out.println(cFireInsuranceRegistration.checkCategory(inputText[0].getText()));
						putText();
						dispose();
					} else {
